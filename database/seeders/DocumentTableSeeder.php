...
class DocumentTableSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        foreach($documentsEn as $key => $document)
            $this->createDocument($document, 'en', $key +1);
        foreach($documentsFa as $key => $document)
            $this->createDocument($document, 'fa', $key +1);
    }
    ...
    public function createDocument($data, $lang, $i)
    {
        ...
        // $i = rand(1, 6);
        $img = public_path("general/img/intro.docx");
        if (file_exists($img)) {
            $fakeUploadedFile = UploadedFile::fake()->createWithContent("intro.docx", file_get_contents($img));
            $image =  $this->createFakeImage($fakeUploadedFile, $document, 'attachment');
        }
    }
}
