    ...
    <div class="card">
        <div class="card-body">
            <div class="d-flex justify-content-between mb-2">
                ...
                <button class="btn btn-sm btn-info" wire:click="create">{{ __('global.create_document') }}</button>
            </div>
                ...
                <table class="table">
                    ...
                    <tbody>
                        @foreach ($documents as $key => $document)
                            ...
                            <tr>
                                <th scope="row">{{  ($documents->currentPage()-1) * $documents->perPage() + $key + 1 }}</th>
                                <td wire:click="edit({{ $document->id }})" class="cursor-pointer text-nowrap">{{ $document->title ?: "_" }}</td>
                                <td wire:click="edit({{ $document->id }})" class="cursor-pointer text-nowrap">{{ $document->slug }}</td>
                                {{-- <td wire:click="edit({{ $document->id }})" class="cursor-pointer text-nowrap">{{ \App\Enums\EnumLanguages::trans($document->lang) }}</td> --}}
                                <td wire:click="edit({{ $document->id }})" class="cursor-pointer text-nowrap">{{ $document->createdBy?->full_name ?: "-" }}</td>
                                <td class="text-nowrap">{{ \Morilog\Jalali\Jalalian::fromDateTime($document->created_at)->format('Y-m-d') }}</td>
                                <td class="text-nowrap">
                                    <div class="form-check form-switch">
                                        <input class="form-check-input" type="checkbox" id="flexSwitchCheckDefault" @checked($document->is_active) wire:click="changeActiveStatus({{ $document->id }})">
                                    </div>
                                </td>
                                <td class="text-nowrap">
                                    <i class="cursor-pointer ti ti-trash text-danger ms-2" data-bs-toggle="tooltip" data-bs-placement="top" onclick="Custom.deleteItemList({{$document->id}})" title="{{ __('global.delete') }}"></i>
                                    <i class="cursor-pointer ti ti-pencil text-warning ms-2" data-bs-toggle="tooltip" data-bs-placement="top" wire:click="edit({{ $document->id }})" title="{{ __('global.edit') }}"></i>
                                    <i class="cursor-pointer ti ti-cloud-download" data-bs-toggle="tooltip" data-bs-placement="top" wire:click="download({{ $document->id }})" title="{{ __('global.download') }}"></i>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
