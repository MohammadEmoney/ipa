    ...
    <div class="card">
        <div class="card-body">
            <div class="d-flex justify-content-between mb-2">
                ...
                <button class="btn btn-sm btn-info" wire:click="create">{{ __('global.create_group_layout') }}</button>
            </div>
            <div class="table-responsive">
                <div class="row">
                    <div class="col-md-4">
                        <input type="text" class="form-control blayoutGroup-info" wire:model.live.debounce.600="search" placeholder="{{ __('global.search') }} ...">
                    </div>
                    <div class="col-md-2">
                    ...
                    <div class="col-md-12">
                        <div id="collapseFilter" class="accordion-collapse collapse" aria-labelledby="headingFilter" data-bs-parent="#accordionExample" wire:ignore.self>
                            <div class="accordion-body">
                                ...
                                {{-- <div class="row p-3">
                                    <div class="blayoutGroup col-md-6 px-0 py-2 text-center">
                                        <span>نوع سفارش:</span>
                                        <div class="form-check form-check-inline">
                                            <input class="form-check-input" type="radio" wire:model.live="filter.layoutGroup_type" name="layoutGroupType" id="inlineRadio1" value="tuition">
                                            <label class="form-check-label" for="inlineRadio1">نمایش شهریه ها</label>
                                        </div>
                                        <div class="form-check form-check-inline">
                                            <input class="form-check-input" type="radio" wire:model.live="filter.layoutGroup_type" name="layoutGroupType" id="inlineRadio2" value="book">
                                            <label class="form-check-label" for="inlineRadio2">نمایش کتاب ها</label>
                                        </div>
                                        <div class="form-check form-check-inline">
                                            <input class="form-check-input" type="radio" wire:model.live="filter.layoutGroup_type" name="layoutGroupType" id="inlineRadio3" value="">
                                            <label class="form-check-label" for="inlineRadio3">نمایش همه</label>
                                        </div>
                                    </div>
                                    <div class="blayoutGroup col-md-6 p-2 text-center">
                ...
                <table class="table">
                    ...
                    <tbody>
                        @foreach ($layoutGroups as $key => $layoutGroup)
                            <tr>
                                <th scope="row">{{  ($layoutGroups->currentpage()-1) * $layoutGroups->perpage() + $key + 1 }}</th>
                                <td wire:click="edit({{ $layoutGroup->id }})" class="cursor-pointer text-nowrap">{{ $layoutGroup->title ?: "_" }}</td>
                                <td wire:click="edit({{ $layoutGroup->id }})" class="cursor-pointer text-nowrap">{{  number_format($layoutGroup->count_list) }}</td>
                                <td wire:click="edit({{ $layoutGroup->id }})" class="cursor-pointer text-nowrap">{{ \App\Enums\EnumLanguages::trans($layoutGroup->lang) }}</td>
                                <td wire:click="edit({{ $layoutGroup->id }})" class="cursor-pointer text-nowrap">{{ \App\Enums\EnumLayoutGroupTags::trans($layoutGroup->tag) }}</td>
                                <td class="text-nowrap">{{ \Morilog\Jalali\Jalalian::fromDateTime($layoutGroup->created_at)->format('Y-m-d') }}</td>
                                <td class="text-nowrap">
                                    <div class="form-check form-switch">
                                        <input class="form-check-input" type="checkbox" id="flexSwitchCheckDefault" @checked($layoutGroup->is_active) wire:click="changeActiveStatus({{ $layoutGroup->id }})">
                                    </div>
                                </td>
                                <td class="text-nowrap">
                                    <i class="cursor-pointer ti ti-trash text-danger ms-2" data-bs-toggle="tooltip" data-bs-placement="top" onclick="Custom.deleteItemList({{$layoutGroup->id}})" title="{{ __('global.delete') }}"></i>
                                    <i class="cursor-pointer ti ti-pencil text-warning ms-2" data-bs-toggle="tooltip" data-bs-placement="top" wire:click="edit({{ $layoutGroup->id }})" title="{{ __('global.edit') }}"></i>
                                    <i class="cursor-pointer ti ti-list-details" data-bs-toggle="tooltip" data-bs-placement="top" wire:click="show({{ $layoutGroup->id }})" title="{{ __('global.layouts') }}"></i>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
