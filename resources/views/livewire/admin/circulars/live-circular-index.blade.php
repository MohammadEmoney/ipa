    ...
    <div class="card">
        <div class="card-body">
            <div class="d-flex justify-content-between mb-2">
                ...
                <button class="btn btn-sm btn-info" wire:click="create">{{ __('global.create_circular') }}</button>
            </div>
                    ...
                    <div class="col-md-12">
                        <div id="collapseFilter" class="accordion-collapse collapse" aria-labelledby="headingFilter" data-bs-parent="#accordionExample" wire:ignore.self>
                            <div class="accordion-body">
                                ...
                                {{-- <div class="row p-3">
                                    <div class="bcircular col-md-6 px-0 py-2 text-center">
                                        <span>نوع سفارش:</span>
                                        <div class="form-check form-check-inline">
                                            <input class="form-check-input" type="radio" wire:model.live="filter.circular_type" name="circularType" id="inlineRadio1" value="tuition">
                                            <label class="form-check-label" for="inlineRadio1">نمایش شهریه ها</label>
                                        </div>
                                        <div class="form-check form-check-inline">
                                            <input class="form-check-input" type="radio" wire:model.live="filter.circular_type" name="circularType" id="inlineRadio2" value="book">
                                            <label class="form-check-label" for="inlineRadio2">نمایش کتاب ها</label>
                                        </div>
                                        <div class="form-check form-check-inline">
                                            <input class="form-check-input" type="radio" wire:model.live="filter.circular_type" name="circularType" id="inlineRadio3" value="">
                                            <label class="form-check-label" for="inlineRadio3">نمایش همه</label>
                                        </div>
                                    </div>
                                    <div class="bcircular col-md-6 p-2 text-center">
                ...
                <table class="table">
                    ...
                    <tbody>
                        @foreach ($circulars as $key => $circular)
                            ...
                            <tr>
                                <th scope="row">{{  ($circulars->currentPage()-1) * $circulars->perPage() + $key + 1 }}</th>
                                <td wire:click="edit({{ $circular->id }})" class="cursor-pointer text-nowrap">{{ $circular->title ?: "_" }}</td>
                                <td wire:click="edit({{ $circular->id }})" class="cursor-pointer text-nowrap">{{ $circular->slug }}</td>
                                {{-- <td wire:click="edit({{ $circular->id }})" class="cursor-pointer text-nowrap">{{ \App\Enums\EnumLanguages::trans($circular->lang) }}</td> --}}
                                <td wire:click="edit({{ $circular->id }})" class="cursor-pointer text-nowrap">{{ $circular->createdBy?->full_name ?: "-" }}</td>
                                <td class="text-nowrap">{{ \Morilog\Jalali\Jalalian::fromDateTime($circular->created_at)->format('Y-m-d') }}</td>
                                <td class="text-nowrap">
                                    <div class="form-check form-switch">
                                        <input class="form-check-input" type="checkbox" id="flexSwitchCheckDefault" @checked($circular->is_active) wire:click="changeActiveStatus({{ $circular->id }})">
                                    </div>
                                </td>
                                <td class="text-nowrap">
                                    <i class="cursor-pointer ti ti-trash text-danger ms-2" data-bs-toggle="tooltip" data-bs-placement="top" onclick="Custom.deleteItemList({{$circular->id}})" title="{{ __('global.delete') }}"></i>
                                    <i class="cursor-pointer ti ti-pencil text-warning ms-2" data-bs-toggle="tooltip" data-bs-placement="top" wire:click="edit({{ $circular->id }})" title="{{ __('global.edit') }}"></i>
                                    <i class="cursor-pointer ti ti-cloud-download" data-bs-toggle="tooltip" data-bs-placement="top" wire:click="download({{ $circular->id }})" title="{{ __('global.download') }}"></i>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
