...
class CircularTableSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        $circularsEn = [
            [
                ...
                'description' => '<div class="rich-text">
                    ...
                    <p>This briefing also introduces a theoretical background on human perception, cognition, and a model of situational awareness for pilot monitoring tasks. It then describes five tools designed to enhance monitoring effectiveness and their real-world applications. The reader is invited either to review the complete circular or to review the tools individually, with references to the theoretical background where necessary.</p>
                </div>',
            ],
            [
        ...
        foreach($circularsEn as $key => $circular)
            $this->createDocument($circular, 'en', $key +1);
        foreach($circularsFa as $key => $circular)
            $this->createDocument($circular, 'fa', $key +1);
    }

    public function createDocument($data, $lang, $i)
    {
        ...
        $i = rand(1, 3);
        $img = public_path("Impact/assets/img/seed/$i.jpg");
        if (file_exists($img)) {
            $fakeUploadedFile = UploadedFile::fake()->createWithContent("$i.jpg", file_get_contents($img));
            $this->createFakeImage($fakeUploadedFile, $circular, 'attachment');
        }
        $img = public_path("Impact/assets/img/seed/$i.jpg");
        if (file_exists($img)) {
            $fakeUploadedFile = UploadedFile::fake()->createWithContent("$i.jpg", file_get_contents($img));
            $this->createFakeImage($fakeUploadedFile, $circular, 'mainImage');
        }
    }
}
