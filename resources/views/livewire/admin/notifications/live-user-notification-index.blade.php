                ...
                <table class="table">
                    <thead>
                        <tr>
                            ...
                            <th scope="col">{{ __('global.send_via') }}</th>
                            <th scope="col">{{ __('global.from') }}</th>
                            <th scope="col">{{ __('global.send_to') }}</th>
                    ...
                    <tbody>
                        @foreach ($notifications as $key => $notification)
                            ...
                            <tr>
                                ...
                                <td wire:click="edit({{ $notification->id }})" class="cursor-pointer text-nowrap">{{ $notification->title ?: "_" }}</td>
                                <td>{{ $notification->user?->full_name ?: "-" }}</td>
                                <td>{{ $notification->send_via_as_string ?: "-" }}</td>
                                <td>{{ $notification->from ?: "-" }}</td>
                                <td>{{ $notification->send_to_as_string ?: "-" }}</td>
                                ...
                                <td class="text-nowrap">
                                    ...
                                    <i class="cursor-pointer ti ti-eye text-warning ms-2" data-bs-toggle="tooltip" data-bs-placement="top" wire:click="show({{ $notification->id }})" title="{{ __('global.show') }}"></i>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
